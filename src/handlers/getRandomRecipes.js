const {getRecipePreviewByData,getRandomRecipeData} = require("../shared/utils")

const getRandomRecipe = async (req, res, next) => {
    try {
      // for random recipes https://api.spoonacular.com/recipes/random
      const randomRecipesAPI = await getRandomRecipeData(req.body.count); // get the recipe info by given id
      const previews = [];
